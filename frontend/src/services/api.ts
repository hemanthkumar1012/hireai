...
export const profilesApi = {
  ...
  async parseResume(resumeText: string): Promise<JobSeekerProfile> {
    const res = await apiClient.post('/profiles/me/parse-resume', {
  ...
  async analyzeResume(
    ...
    filename: string;
    analysis: JobSeekerProfile['resume_analysis'];
  }> {
...
export const applicationsApi = {
  ...
  async listRecruiter(jobId?: number): Promise<Application[]> {
    const res = await apiClient.get('/applications/recruiter', {
      params: {
        job_id: jobId,
      },
    });
  ...
  async updateStatus(
    ...
    const res = await apiClient.patch(
      `/applications/${id}/status?status=${newStatus}`
    );
