...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#070A0F] text-slate-100 overflow-hidden font-sans">
          ...
          >
            ...
            {!isCollapsed && (
              <div className="truncate">
                ...
                <p className="text-[9px] text-slate-500 font-bold mt-1.5 uppercase tracking-wider truncate capitalize">
                  {user?.role}
                </p>
              </div>
            )}
      ...
      {sidebarOpen && (
        <div
          className="fixed inset-0 z-50 flex md:hidden bg-black/60 backdrop-blur-sm"
          onClick={() => setSidebarOpen(false)}
        >
          <aside
            className="w-64 bg-brand-surface1 border-r border-brand-border flex flex-col h-full"
            ...
            <div className="p-4 border-t border-brand-border bg-[#070A0F]/20">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="truncate">
                  ...
                  <p className="text-[9px] text-slate-500 uppercase tracking-wider capitalize">
                    {user?.role}
                  </p>
                </div>
              </div>
        ...
        <header className="h-16 flex items-center justify-between px-6 border-b border-brand-border bg-brand-bg/60 backdrop-blur-md z-10">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              title="Notifications"
              className="min-h-10 min-w-10 p-2 hover:bg-brand-surface1 border border-brand-border hover:border-slate-700 rounded-xl text-slate-400 hover:text-white transition relative"
            >
